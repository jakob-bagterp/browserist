...
def combo_log_in(browser_driver: BrowserDriver, login_credentials: LoginCredentials, login_form: LoginForm1Step | LoginForm2Steps, timeout: int) -> None:
    # TODO: Incorporate timeout strategy and settings

    def login_form_1_step(browser_driver: BrowserDriver, login_form: LoginForm1Step, timeout: int) -> None:
    ...
    def login_form_2_steps(browser_driver: BrowserDriver, login_form: LoginForm2Steps, timeout: int) -> None:
        input_value(browser_driver, login_form.username_input_xpath, login_credentials.username, timeout)
    ...
    if type(login_form) is LoginForm1Step:
        login_form_1_step(browser_driver, login_form, timeout)
    elif type(login_form) is LoginForm2Steps:
        login_form_2_steps(browser_driver, login_form, timeout)
