...
class WaitForPageTitleToChangeTimeoutException(Exception):
    def __init__(self, driver: object, page_title_fragment: str) -> None:
        current_url = get_current_url(driver)
        current_page_title = get_current_page_title(driver)
        self.message = f"On page {current_url} with title \"{current_page_title}\", check of page title fragment timed out, or the title fragment doesn't match: {page_title_fragment}"
        super().__init__(self.message)
