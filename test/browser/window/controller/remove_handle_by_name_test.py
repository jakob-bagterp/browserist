...
@pytest.mark.parametrize("name", [(WINDOW_HANDLE_2_NAME)])
@pytest.mark.xdist_group(name="serial_window_tests")
def test_window_handle_controller_remove_handle_by_name(
    name: str, window_handle_controller: WindowHandleController
) -> None:
...
@pytest.mark.xdist_group(name="serial_window_tests")
def test_window_handle_controller_remove_handle_by_name_invalid_error(
...
@pytest.mark.parametrize(
    "name, expectation",
    [(WINDOW_HANDLE_2_NAME, does_not_raise()), (WINDOW_HANDLE_4_NAME, pytest.raises(WindowHandleNameNotFoundError))],
)
@pytest.mark.xdist_group(name="serial_window_tests")
def test_window_handle_controller_remove_handle_by_name_not_found_error(
    name: str, expectation: Any, window_handle_controller: WindowHandleController
) -> None:
    with expectation:
        _ = window_handle_controller.remove_handle_by_name(name) is not None
