...
MINIMUM_FILE_SIZE_SCROLL_VERTICAL_LONG = 400_000  # Merge of multiple, e.g. 10-15, files.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (internal_url.SCROLL_VERTICAL_LONG, MINIMUM_FILE_SIZE_SCROLL_VERTICAL_LONG),
    ],
)
@pytest.mark.xdist_group(name="serial_screenshot_tests")
def test_default_get_screenshot_of_complete_page_1(
    url: str, minimum_file_size: int, browser_default_headless_screenshot: Browser, tmpdir: Path
) -> None:
...
@pytest.mark.xdist_group(name="serial_screenshot_tests")
def test_default_get_screenshot_of_complete_page_2(
    url: str, minimum_file_size: int, browser_default_headless_screenshot: Browser, tmpdir: Path
) -> None:
...
@pytest.mark.xdist_group(name="serial_screenshot_tests")
def test_default_get_screenshot_of_complete_page_3(
    url: str, minimum_file_size: int, browser_default_headless: Browser, tmpdir: Path
) -> None:
...
@pytest.mark.xdist_group(name="serial_screenshot_tests")
def test_default_get_screenshot_of_complete_page_4(
    url: str, minimum_file_size: int, browser_default_headless: Browser, tmpdir: Path
) -> None:
