...
import _helper
import pytest
from _config.combo.cookie_banner import (
...
def test_combo_cookie_banner(
    cookie_banner_settings: CookieBannerSettings, browser_default_headless_disable_images: Browser
) -> None:
...
@pytest.mark.xdist_group(name="serial_combo_cookie_banner_tests")
def test_combo_cookie_banner_return_iframe_to_origin(browser_default_headless_disable_images: Browser) -> None:
    with expectation_of_no_exceptions_raised():
        browser = reset_to_not_timed_out(browser_default_headless_disable_images)
...
def test_combo_cookie_banner_has_loaded_wait_seconds(
    ...
    def accept_cookie_banner_and_get_time(
        browser: Browser, cookie_banner_settings: CookieBannerSettings, has_loaded_wait_seconds: float
    ) -> float:
...
def test_combo_cookie_banner_has_disappeared_wait_seconds(
    ...
    def accept_cookie_banner_and_get_time(
        browser: Browser, cookie_banner_settings: CookieBannerSettings, has_disappeared_wait_seconds: float
    ) -> float:
    ...
    with expectation_of_no_exceptions_raised():
        ...
        time_measured_a = accept_cookie_banner_and_get_time(
            browser, cookie_banner_settings, has_disappeared_wait_seconds_a
        )
        time_measured_b = accept_cookie_banner_and_get_time(
...
@pytest.mark.parametrize(
    "cookie_banner_settings, expected_state",
    [
...
def test_combo_cookie_banner_state(
    cookie_banner_settings: CookieBannerSettings, expected_state: bool, browser_default_headless_disable_images: Browser
) -> None:
    browser = reset_to_not_timed_out(browser_default_headless_disable_images)
